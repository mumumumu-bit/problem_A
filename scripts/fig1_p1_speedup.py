...
data_min, data_max = min(m1), max(m1)
ax.set_ylim(bottom=data_min-0.3, top=data_max+max(s1)+0.3)
ax.margins(x=0.01)
...
fig.suptitle("图1:P1多核加速比曲线(100个测试用例)",fontsize=22,fontweight="bold",x=0.5,y=0.03,ha="center")
fig.tight_layout(rect=[0,0.06,1,1],pad=0.5)
O=ROOT/"results/final_visualization/figures"; O.mkdir(parents=True,exist_ok=True)
for f in["pdf","png","svg"]: fig.savefig(O/f"fig1_p1_speedup.{f}",dpi=600,bbox_inches="tight",pad_inches=0.1)
print("图1 OK"); plt.close()
