...
本文曾在开发阶段探索SA-VNS，但已有探索实验未显示稳定的额外收益，因此未纳入stable solver/full100正式算法，最终方法仍采用严格下降VNS。详细探索结果见附录C。
...
算法属于预算约束下的启发式搜索，不保证全局最优。官方Evaluator调用成本限制可探索候选数量，严格下降也可能无法穿越非改进区域。不同case的依赖及通信结构可能影响可用改善空间，正式收益分布表明case间幅度不同，但并未识别这些结构因素的独立贡献。
