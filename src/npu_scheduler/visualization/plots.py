...
def plot_benchmark(directory):
    ...
    if controlled.exists():
        ...
        fig,ax=plt.subplots()
        ax.plot(ns,[1]*len(ns),'o--',color=COLORS['no_l2'],label='No L2, same plan')
        ax.plot(ns,values,'s-',color=COLORS['l2'],label='L2, same plan')
        ax.set(xlabel='Number of cores',ylabel='Mean no-L2 / L2 makespan',xticks=ns); ax.legend()
        save(fig,figures,'p3_same_plan_cache'); plt.close(fig)
